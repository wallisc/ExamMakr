...
/**
 * View for matching questions in an exam.
 */
public class MatchingView extends QuestionView
{
    /**
     * Creates new form MatchingView
     */
    public MatchingView()
    {
        super();
        item = Lookup.getDefault().lookup(MatchingApi.class).newItem();
        setEditView(false);
        initComponents();
        answerPanel.setLayout(new GridLayout(0, 1));
        answerPanel.add(new MatchingAnswerPanel(this));
        fillFields();
     ...
     */
    public MatchingView(MatchingApi nItem) 
    {
        super();
        item = nItem;
        setEditView(true);
    ...
    {
        MatchingApi mQuestion = (MatchingApi)inItem;
        questionHeaderPanel.fillFields(mQuestion);
        getAnswerPanel().fillFields(mQuestion); 
        examNavigatorPanel.refreshView(isEditView());
    }

    protected MatchingAnswerPanel getAnswerPanel()
    {
        return (MatchingAnswerPanel)(answerPanel.getComponent(0));
    }

    /**
     * Returns a MatchingChoice constructed from the fields in the GUI
     * @return MatchingChoice constructed from the GUI fields
     */
    @Override
    protected MatchingApi extractItem()
    {
        MatchingApi toAdd = Lookup.getDefault().lookup(
                MatchingApi.class).newItem();
        toAdd.removePairs();
